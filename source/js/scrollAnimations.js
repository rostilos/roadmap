...
/*  
        ...
        */
const animatedElements = document.querySelectorAll("[data-animate-component]");
Array.from(animatedElements).forEach((element) => {
  ...
  function calculateStylesOnScroll() {
    /*
                ...
                */
    const { startPositionY, endPositionY, animationLenghtX, animationLenghtY } =
    ...
    if (isStartAnimationPointReached) {
      ...
      /*
                    ...
                    */
      deltaX = (animationLenghtX * multipliedDeltaY) / animationLenghtY;
      ...
      if (animationRulesArray.includes("fade-in")) {
        let opacity =
          (animationTreshold - (endPositionY - animationLenghtY)) /
          (animationTreshold * 2);
        let right = Math.max(
          ((endPositionY - animationLenghtY) / animationTreshold) * 1000,
          0
        );
...
function calculatePosValues(element) {
  ...
  const startPositionY =
    animatedElementsPositions[animateComponent]["start"].getBoundingClientRect()
      .y;
  const endPositionY =
    animatedElementsPositions[animateComponent]["end"].getBoundingClientRect()
      .y;
